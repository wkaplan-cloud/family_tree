  ...
  async function handleAddRelative() {
    ...
    try {
      ...
      if (form.alive && form.email.trim()) {
        ...
        if (inviteRes.ok) {
          inviteMessage = ` An invitation email was sent to ${form.email.trim()}.`;
        } else {
          inviteMessage = ` The relative was added, but the invitation email could not be sent.`;
        }
      }
      ...
      setNotice(`${created.person.name} was added to ${selectedPerson.name}'s branch.${inviteMessage}`.trim());
      await loadGraph();
    } catch (err) {
      console.error(err);
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <ol className="mt-3 space-y-3 text-sm text-slate-700">
                ...
                <li>3. Add an email if you want an invite sent automatically.</li>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4">
          <div className="w-full max-w-xl rounded-[28px] bg-white p-6 shadow-2xl">
              ...
              {form.alive && (
                <div>
                  ...
                  <p className="mt-2 text-xs leading-5 text-slate-500">
                    If you add an email here, the site will automatically send this relative an invite link after you save.
                  </p>
                </div>
              )}
            ...
            <div className="mt-6 flex gap-3">
              ...
              >
                {saving ? "Saving..." : form.email.trim() ? "Save and send invite" : "Save relative"}
              </button>
            </div>
          </div>
        </div>
